		...
		$.post(url2,data,function(result){
	        if(result.state==0){
	        	...
	        	for(var i=0;i<courseList.length;i++){
	                ...
	                '</center></td><td><center>' +
	                courseScore+
	                '<td><center><input  type="button" value="查看评估" onclick="lookteaEvaluate(\''+courseNo+'\')"></center></td>'+
	                '</center></td><tr>';
	              tb.append(tr);
	        	}
...
function lookteaEvaluate(courseNo){
	location.href='tea_look_courseval.html?courseNo='+courseNo;
}
