		...
		$.post(url2,data,function(result){
	        if(result.state==0){
	        	...
	        	for(var i=0;i<courseList.length;i++){
	                ...
	                '</center></td><td><center>' +
	                courseScore+
	                '<td><center><input  type="button" value="查看评估" onclick="lookCourseInfo(\''+courseNo+'\')"></center></td>'+
	                '</center></td><tr>';
	              tb.append(tr);
	        	}
...
function lookCourseInfo(courseNo){
	location.href='teacher_look_evaluate.html?courseNo='+courseNo;
}
